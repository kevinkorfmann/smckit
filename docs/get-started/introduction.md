...
smckit has a deliberate install boundary:

- a wheel install is supposed to be enough for the packaged native quickstarts
  and teaching/demo workflows
- a source checkout is the supported path for full upstream-preservation work,
  especially when the original tool still lives in vendored source, R, Java,
  D, or other non-wheel runtimes
